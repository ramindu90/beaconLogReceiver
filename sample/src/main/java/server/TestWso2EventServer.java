...
private boolean eventReceived = false;
    ...
    private AtomicLong msgCount2 = new AtomicLong(0);
    private AtomicLong exceedingCount = new AtomicLong(0);
    private int listeningPort;
    private List<Event> preservedEventList = null;
    private boolean isPreservingEvents;
    private int elapseCount = 1000;
    private long totalLatancy = 0;
...
synchronized (testServer) {
 ...
 eventReceived = true;
                ...
                for (Event event : eventList) {
                    long timestamp = (Long) event.getPayloadData()[2];
                    long currentTimestamp = System.currentTimeMillis();
                    long eventLatency = currentTimestamp - timestamp;

                    totalLatancy = totalLatancy + eventLatency;

                    msgCount2.incrementAndGet();

                    if(eventLatency > 5){
                        exceedingCount.incrementAndGet();
                    }

                    if (msgCount2.get() % elapseCount == 0) {
                        double latency=totalLatancy;
                        totalLatancy = 0;
                        double avgLatency = ( latency) / elapseCount ;
                        double throughput = (((double) elapseCount) / (currentTimestamp - tempstartTime * 1.0)) * 1000;

                        log.info("Sent " + elapseCount + " events with a throughput of " + throughput +
                                 " events per second || and latency: " + avgLatency + "|| exceededPercentage: " + exceedingCount.get()*100.0/elapseCount);
                        tempstartTime = currentTimestamp;
                        exceedingCount = new AtomicLong(0);
